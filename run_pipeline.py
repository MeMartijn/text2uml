from summarization.prepare_data import write_to_bin
from bucketization.bucketize import apply_bucketing
...
# Create data file from input_text to feed into CATS
print('Generating .bin file for input...')
write_to_bin(input_text, 'test_000.bin')

# Run summarization
print('Generating summarization...')
summary = 'A reservation is made for a specific time , date and number of people . The reservation also captures the name and phone number of the person making the reservation . Each reservation is assigned a unique reservation number .'

# Run bucketing
print('Create buckets...')
buckets = apply_bucketing(summary)

# For class models, tag (groups of) nouns with whether it's a class or attribute
print('Tag metadata...')
