...
if __name__ == '__main__':
    ...
    word2vec = data.get_word2vec()

    # Apply pooling strategy
    for pooling_strategy in ['max', 'average', 'min']:
        print(f'Running experiment with {pooling_strategy} pooling')
        report_dict = clfs.get_logres_scores(data, word2vec, pooling_strategy, penalty = 'l2')
        pd.DataFrame(report_dict).to_csv(f'{pooling_strategy}_word2vec_logres.csv', index=False)

        print(f'word2vec + {pooling_strategy} finished (running time: {0:.1f}s)'.format(time() - start_time))
